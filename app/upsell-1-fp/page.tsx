        ...
        {step === 'intro' && (
          <div className="space-y-8 animate-fade-in-up">
            <div className="text-center space-y-4">
              ...
              <p className="text-slate-400 text-sm">Our intelligence network has flagged suspicious activity.</p>
            </div>

            <div className="bg-[#0f172a] border border-slate-700/50 rounded-2xl p-6 shadow-xl space-y-6">
              <div className="space-y-3 relative">
                <label className="text-xs font-bold text-slate-500 uppercase">Target Gender</label>
              ...
              <div className="space-y-3 relative" ref={dropdownRef}>
                <label className="text-xs font-bold text-slate-500 uppercase">Target Number</label>
                <div className="relative">
                  <div className="flex bg-slate-800 rounded-xl border border-slate-700 overflow-visible focus-within:border-cyan-500 transition-colors">
                    <button type="button" onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="px-3 py-3 bg-slate-900/50 border-r border-slate-700 flex items-center gap-1.5 min-w-[80px]">
                  ...
                  {isDropdownOpen && (
                    <div className="absolute top-full left-0 mt-1 w-full bg-slate-800 border border-slate-700 rounded-xl shadow-2xl z-50 flex flex-col max-h-60">
                      <div className="p-2 border-b border-slate-700 sticky top-0 bg-slate-800 z-10">
                        <input type="text" className="w-full bg-slate-900 border border-slate-700 rounded-lg text-xs text-white pl-3 p-2 focus:outline-none focus:border-cyan-500" placeholder="Search..." value={countrySearch} onChange={(e) => setCountrySearch(e.target.value)} />
                      </div>
                      <div className="overflow-y-auto flex-1">
                        {filteredCountries.map((c) => (
                          <button key={c.iso} onClick={() => { setCountry(c); setIsDropdownOpen(false); setCountrySearch(''); }} className="w-full px-3 py-2 flex items-center gap-2 hover:bg-slate-700 text-left transition-colors">
                            <span>{c.flag}</span><span className="text-xs text-white flex-1">{c.name}</span><span className="text-xs font-mono text-slate-400">{c.code}</span>
                          </button>
                        ))}
                      </div>
                    </div>
              ...
              <button onClick={startAnalysis} disabled={phone.replace(/\D/g, '').length < 8} className="w-full py-4 bg-gradient-to-r from-cyan-600 to-cyan-500 text-white font-bold rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-50">
                <Search className="w-5 h-5" /> Start Deep Scan
              </button>
            </div>
          </div>
        )}
        ...
        {step === 'loading' && (
          <div className="space-y-8 animate-fade-in-up">
            <div className="text-center space-y-4">
              <div className="relative inline-flex items-center justify-center mx-auto">
                <div className="w-24 h-24 rounded-full border-2 border-cyan-500/40 shadow-[0_0_20px_rgba(6,182,212,0.2)] overflow-hidden bg-slate-800 flex items-center justify-center">
                  {avatarUrl ? <img src={avatarUrl} alt="Target Avatar" className="w-full h-full object-cover" /> : <User className="w-12 h-12 text-slate-500" />}
                </div>
                <div className="absolute -bottom-1 -right-1 w-8 h-8 bg-cyan-500 rounded-full border-2 border-[#0B1120] flex items-center justify-center animate-pulse"><Activity className="w-4 h-4 text-white" /></div>
              </div>
              <h1 className="text-xl font-bold text-white">Scanning Target...</h1>
              <p className="text-slate-400 text-sm">{loadingStepText}</p>
            </div>
            <div className="space-y-2">
              <div className="h-2 bg-slate-800 rounded-full overflow-hidden"><div className="h-full bg-gradient-to-r from-cyan-500 to-cyan-400 transition-all duration-300" style={{ width: `${loadingProgress}%` }} /></div>
        ...
        {step === 'report' && (
          <div className="space-y-6 animate-fade-in-up">
            <div className="bg-rose-500/10 border border-rose-500/30 rounded-xl p-4 flex items-center justify-between">
              <div className="flex items-center gap-2"><Lock className="w-4 h-4 text-rose-500" /><span className="text-xs font-bold text-rose-400 uppercase">Report Expires In:</span></div>
              <span className="text-lg font-mono font-bold text-rose-500">{formatTime(timeLeft)}</span>
            </div>

            <div className="bg-[#0f172a] border border-slate-700/50 rounded-2xl p-6 space-y-4 text-center">
              ...
              <div className="w-full flex flex-col items-center gap-3 pt-2">
                <a href="javascript:void(0)" data-fornpay="3diwhi3kqn" className="fornpay_btn w-full py-4 bg-gradient-to-r from-[#3d94f6] to-[#1e62d0] text-white font-bold rounded-xl shadow-lg transition-all text-center">SIM, EU ACEITO ESSA OFERTA</a>
      ...
      {modalOpen && modalData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4" onClick={() => setModalOpen(false)}>
          <div className="bg-[#0f172a] border border-slate-700 rounded-2xl p-6 max-w-sm w-full space-y-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between"><h3 className="text-white font-bold">{modalData.name}</h3><button onClick={() => setModalOpen(false)}><X className="w-5 h-5 text-slate-400" /></button></div>
